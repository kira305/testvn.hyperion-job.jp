...
class LC_Page_User extends LC_Page_Ex {
    ...
    function action() {
        ...
        switch ($this->mode) {
            ...
            case 'delete':
                $tempList = array();
                foreach ($_SESSION['keep_product_list'] as $key => $value) {
                    if ($product_id != $value) {
                        $tempList[] = $value;
                    }
                }
                $_SESSION['keep_product_list'] = $tempList;
                break;
            default:
                break;
        }
        if (isset($_SESSION['keep_product_list']) && count($_SESSION['keep_product_list']) > 0) {
            ...
            $this->tpl_linemax = count($firstKept);
            ...
            $objQuery->setWhere('product_id IN (' . SC_Utils_Ex::sfGetCommaList($firstKept) . ')');
