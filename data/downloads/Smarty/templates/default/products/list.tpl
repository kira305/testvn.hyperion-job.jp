...
<div id="undercolumn">
    <form name="form1" id="form1" method="get" action="?">
        ...
        <!--{* ▼検索条件 *}-->
        <input type="hidden" name="work_location_flg" value="<!--{$arrSearchData.work_location_flg|h}-->" />
        <input type="hidden" name="employment_status" value="<!--{$arrSearchData.employment_status|h}-->" />
    ...
    <!--{foreach from=$arrProducts item=arrProduct name=arrProducts}-->
        ...
        <!--{if $smarty.foreach.arrProducts.first}-->
            <!--▼件数-->
            <div>
                <span class="attention"><!--{$tpl_linemax}-->件</span>の求人がございます。
            </div>
            <!--▲件数-->
