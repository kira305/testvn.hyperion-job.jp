...
<dd>
    ...
    <!--{/if}-->
    <input type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|h}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" class="boxHarf text data-role-none" placeholder="姓" />&nbsp;&nbsp;<input type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|h}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" class="boxHarf text data-role-none" placeholder="名" />
</dd>
...
<dd>
    ...
    <div class="attention"><!--{$arrErr[$key1]}--><!--{$arrErr[$key2]}--></div>
    <!--{/if}-->
    <input type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|h}-->" max="<!--{$smarty.const.ZIP01_LEN}-->" class="boxShort text data-role-none" />&nbsp;－&nbsp;<input type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|h}-->" maxlength="<!--{$smarty.const.ZIP02_LEN}-->" class="boxShort text data-role-none" />&nbsp;&nbsp;<a href="http://search.post.japanpost.jp/zipcode/" target="_blank" rel="external"><span class="fn" style="font-size: 0.8em">郵便番号検索</span></a>

    <a href="javascript:fnCallAddress('<!--{$smarty.const.INPUT_ZIP_URLPATH}-->', '<!--{$key1}-->', '<!--{$key2}-->', '<!--{$key3}-->', '<!--{$key4}-->');" class="btn_sub btn_inputzip" style="font-size: 0.9em; width: auto">郵便番号から住所自動入力</a>
</dd>
...
<dd>
    ...
    <!--{/if}-->
    <input type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|h}-->" maxlength="<!--{$smarty.const.TEL_ITEM_LEN}-->" class="boxShort text data-role-none" />&nbsp;－&nbsp;<input type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|h}-->" maxlength="<!--{$smarty.const.TEL_ITEM_LEN}-->" class="boxShort text data-role-none" />&nbsp;－&nbsp;<input type="text" name="<!--{$key3}-->" value="<!--{$arrForm[$key3]|h}-->" maxlength="<!--{$smarty.const.TEL_ITEM_LEN}-->" class="boxShort text data-role-none" />
</dd>
...
<dd>
    ...
    <div class="attention"><!--{$arrErr[$key1]}--><!--{$arrErr[$key2]}--></div>
    <!--{/if}-->
    <input type="text" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|h}-->" class="boxLong text top data-role-none" />
    <input type="text" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|h}-->" class="boxLong text data-role-none" placeholder="確認のため2回入力してください" />
</dd>

<!--{if $emailMobile}-->
<dt>携帯メールアドレス&nbsp;<span class="attention">必須</span></dt>
<dd>
...
<dt>性別&nbsp;<span class="attention">必須</span></dt>
<dd>
    ...
    <ul class="input_sex" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->">
        <li>
            <label for="radiofield000" class="jsc-check-sex">
                <input type="radio" name="<!--{$key1}-->" id="radiofield000" class="sex_radiobox" value="1" <!--{if $arrForm[$key1] != 2}-->checked="checked"<!--{/if}--> >
                       <span class="men">男性</span>
            </label>
        </li>
        <li>
            <label for="radiofield001" class="jsc-check-sex">
                <input type="radio" name="<!--{$key1}-->" id="radiofield001" class="sex_radiobox" value="2" <!--{if $arrForm[$key1] == 2}-->checked="checked"<!--{/if}--> >
...
<dt><span>必須</span> パスワードを忘れた時のヒント</dt>
<dd>
...
<dt>メールマガジン&nbsp;<span class="attention">必須</span></dt>
<dd>
