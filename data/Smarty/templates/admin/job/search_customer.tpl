...
<!--
    ...
    function func_submit(customer_id) {
        ...
        var opner_customer_id = 'add_customer_id';
        var tpl_no = '<!--{$tpl_no}-->';
        if (tpl_no !== '') {
            opner_customer_id = 'edit_customer_id';
            fm1.getElementById("no").value = escape('<!--{$tpl_no}-->');
        }

        fm1.getElementById(opner_customer_id).value = customer_id;

        fm.mode.value = 'select_customer';
        fm.anchor_key.value = 'job_customers';
        fm.submit();
        window.close();
...
<!--{if $smarty.post.mode == 'search'}-->
    ...
    <table class="list">
        ...
        <!--{section name=cnt loop=$arrCustomer}-->
            <!--{assign var=id value=$arrCustomer[cnt].customer_id}-->
            <form name="customer_form<!--{$id|h}-->" action="?" onsubmit="return false;">
                <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                <input type="hidden" name="customer_id" value="<!--{$id|h}-->" />
                <tr>
                    <td><!--{$id|h}--></td>
                    ...
                    <td align="center"><a href="" onClick="return func_submit(<!--{$id|h}-->)">決定</a></td>
                </tr>
            </form>
        <!--{sectionelse}-->
            <tr>
