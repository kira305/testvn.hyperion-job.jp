...
<form name="search_form" id="search_form" method="post" action="?">
    ...
    <table>
	    ...
	    <td colspan="3">
            ...
            <span class="attention"><!--{$arrErr[$key]|h}--></span>
            <!--{html_checkboxes name="$key" options=$arrEMPSTATUS selected=$arrForm[$key].value}-->
            </td>
        </tr>
        <tr>
            <th>地域</th>
	    <td colspan="3">
            <!--{assign var=key value="search_work_location_flg"}-->
            <span class="attention"><!--{$arrErr[$key]|h}--></span>
            <!--{html_checkboxes name="$key" options=$arrWORKLOCATION selected=$arrForm[$key].value}-->
            </td>
        </tr>
        <tr>
            <th>ダミーフラグ</th>
	    <td colspan="3">
            <!--{assign var=key value="search_dummy_flg"}-->
            <span class="attention"><!--{$arrErr[$key]|h}--></span>
            <!--{html_checkboxes name="$key" options=$arrDUMMYFLG selected=$arrForm[$key].value}-->
            </td>
        </tr>
    </table>
    <div class="btn">
...
<form name="form1" id="form1" method="post" action="?">
    ...
    <!--{if count($arrProducts) > 0}-->
        ...
        <table class="list" id="products-search-result">
            <tr>
                <th rowspan="2" style="width: 6%">求人ID</th>
                <th rowspan="2" style="width: 9%">画像</th>
                <th rowspan="2" style="width: 9%">求人コード</th>
                <th rowspan="2" style="width: 6%">給与(円)</th>
                <th rowspan="2" style="width: 8%">薬局名</th>
                <th>求人名</th>
                <th rowspan="2" style="width: 5%">募集人数</th>
                <th rowspan="2" style="width: 5%">種別</th>
                <th rowspan="2" style="width: 5%">ダミーフラグ</th>
                <th rowspan="2" style="width: 5%">編集</th>
                <th rowspan="2" style="width: 5%">確認</th>
                <!--{if $smarty.const.OPTION_CLASS_REGIST == 1}-->
                <th rowspan="2" style="width: 5%">規格</th>
                <!--{/if}-->
                <th rowspan="2" style="width: 5%">削除</th>
                <th rowspan="2" style="width: 5%">複製</th>
                <th rowspan="2" style="width: 5%"><a href="<!--{$smarty.const.ROOT_URLPATH}-->" onclick="fnChangeAction('../mail/index.php'); fnModeSubmit('mail', '', ''); return false;">DM配信</a></th>
                <th rowspan="2" style="width: 5%">カレンダー</th>
            </tr>
            <tr>
            ...
            <tr style="background:<!--{$arrPRODUCTSTATUS_COLOR[$status]}-->;">
                ...
                <td rowspan="2" class="right">
                    <!--{$arrProducts[cnt].price02_min}-->
                    <!--{if $arrProducts[cnt].price02_min != $arrProducts[cnt].price02_max}-->
                    <br />～ <!--{$arrProducts[cnt].price02_max}-->
                    <!--{/if}-->            </td>
                <td rowspan="2"><!--{$arrProducts[cnt].shipping_pharmacy|h}--></td>
                ...
                <!--{assign var=key value=$arrProducts[cnt].dummy_flg}-->
                <td class="menu" rowspan="2"><!--{$arrDUMMYFLG[$key]}--></td>
                ...
                <td class="menu" rowspan="2"><input type="checkbox" value="<!--{$arrProducts[cnt].product_id}-->" name="dm[]" /></td>
                <td class="menu" rowspan="2"><a href="javascript:;" name="add_calendar" onclick="win03('<!--{$smarty.const.ROOT_URLPATH}--><!--{$smarty.const.ADMIN_DIR}-->products/calendar.php?product_id=<!--{$arrProducts[cnt].product_id}-->', 'search', '620', '500'); return false;" >表示</a></td>
            </tr>
            <tr style="background:<!--{$arrPRODUCTSTATUS_COLOR[$status]}-->;">
                <td>
                    <!--{* カテゴリ名 *}-->
