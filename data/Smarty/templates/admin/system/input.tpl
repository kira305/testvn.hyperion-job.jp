...
function agencyChanged(agencySelect){
    if(agencySelect.selectedIndex !== 0){
        var corporateSelect = document.getElementById('corporate');
        ...
        <!--{foreach from=$corporateList key=k item=v}-->
...
<table>
    ...
    <tr>
        <th>会社名</th>
        <td>
            <!--{if $arrErr.agency_id}--><span class="attention"><!--{$arrErr.agency_id}--></span><!--{/if}-->
            <select name="agency_id" onchange="agencyChanged(this);">
                <option value="">選択してください</option>
                <!--{html_options options=$agencyList selected=$arrForm.agency_id}-->
            </select>
        </td>
    </tr>
    <tr>
        <th>薬局名</th>
        <td>
            <!--{if $arrErr.corporate_id}--><span class="attention"><!--{$arrErr.corporate_id}--></span><!--{/if}-->
            <select name="corporate_id" id="corporate">
                <option value="">選択してください</option>
                <!--{html_options options=$corporateList[$arrForm.agency_id] selected=$arrForm.corporate_id}-->
            </select>
        </td>
    </tr>
    <tr>
