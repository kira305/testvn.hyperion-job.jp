...
class SC_Helper_Customer {
    ...
    function sfEditCustomerData($array, $customer_id = null) {
        ...
        if (count($array['job']) > 0) {
            $newval = '';
            foreach ($array['job'] as $job)
                $newval .= $job;
            $array['job'] = $newval;
        }
        if (count($array['incoming_mail_type']) > 0) {
            $newval = '';
            foreach ($array['incoming_mail_type'] as $incoming_mail_type)
                $newval .= $incoming_mail_type;
            $array['incoming_mail_type'] = $newval;
        }
    ...
    function sfCustomerEmailDuplicationCheck($customer_id, $email) {
        ...
        $return
                = strlen($arrResults['email']) >= 1 && $email === $arrResults['email'] || strlen($arrResults['email_mobile']) >= 1 && $email === $arrResults['email_mobile']
        ;
        return $return;
    }
    ...
    function sfGetCustomerData($customer_id, $mask_flg = true) {
        ...
        $arrForm['job'] = str_split($arrForm['job']);
        $arrForm['incoming_mail_type'] = str_split($arrForm['incoming_mail_type']);
    ...
    function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false) {
        ...
        $objFormParam->addParam('受信メール種類設定', 'incoming_mail_type');
    ...
    function sfSetSearchParam(&$objFormParam) {
        ...
        $objFormParam->addParam('希望する職業形態', 'search_job', INT_LEN, 'n', array('MAX_LENGTH_CHECK'));
        $objFormParam->addParam('銀行名', 'search_bank', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('口座名義', 'search_account_holder', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('口座番号', 'search_account_number', INT_LEN, 'n', array('MAX_LENGTH_CHECK'));
    }
    ...
    function sfCheckErrorSearchParam(&$objFormParam) {
        ...
        if (SC_Utils_Ex::sfIsInt($array['search_buy_total_from']) && SC_Utils_Ex::sfIsInt($array['search_buy_total_to']) && $array['search_buy_total_from'] > $array['buy_total_to']
        ) {
