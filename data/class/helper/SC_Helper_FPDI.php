...
//        $this->AddCIDFonts('Gothic', 'KozGoPro-Medium-Acro,MS-PGothic,Osaka', $cw, $c_map, $registry);
        $this->AddCIDFonts('Gothic', 'MS-PGothic', $cw, $c_map, $registry);
    }
    ...
    function FancyTableExtended($data, $w, $align, $space = false, $title = true, $height = 3.5) {
        ...
        foreach ($data as $row) {
            ...
            foreach ($row as $col) {
                $this->y = $y;
                $h = $this->SJISMultiCellForFancyTable($w[$i], $h, $col, 1, $align, 0, 0);
                $i++;
            }
            $j++;
            $this->Ln();
