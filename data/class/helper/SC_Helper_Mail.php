...
class SC_Helper_Mail {
    ...
    function sfSendOrderMail($order_id, $template_id, $subject = '', $header = '', $footer = '', $send = true) {
        ...
        $arrProduct = $objQuery->select('product_id, work_location_flg, sub_comment1, sub_comment3, sub_comment4', 'dtb_products', $where);
        foreach ($arrProduct as $key => $value)
            $arrTplVar->arrProduct[$value['product_id']] = $value;
        $arrTplVar->arrWorkLocationFlg = array(1 => '東京２３区以内', 2 => '東京２３区以外', 3 => '埼玉県', 4 => '千葉県', 5 => 'その他');
...
//        $objSendMail->setTo($bcc = $arrInfo['email01'], $bcc = $arrInfo['company_name']);
    ...
    function sfSendMailmagazine($send_id) {
        ...
        $arrDestinationList = $objQuery->select(
                '*', 'dtb_send_customer', 'send_id = ? AND (send_flag = 2 OR send_flag IS NULL)', array($send_id)
        );
        ...
        foreach ($arrDestinationList as $arrDestination) {
            ...
            $objMail->setItem(
                    $arrDestination['email'], $subjectBody, $mailBody, $objSite['email03'], // 送信元メールアドレス
