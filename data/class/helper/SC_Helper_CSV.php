...
class SC_Helper_CSV {
    ...
    function sfDownloadCsv($csv_id, $where = '', $arrVal = array(), $order = '', $is_download = false) {
        ...
        if ($csv_id == '1') {
            ...
            // クライアントの場合
            $sql = 'SELECT ' . $cols . ' FROM mtb_corporate ' . $where;
        } else if ($csv_id == '7') {
            // クライアントの場合
            $sql = 'SELECT ' . $cols . ' FROM dtb_prepayment_apply ' . $where;
        } else if ($csv_id == '8') {
            // 社員の場合
            $sql = 'SELECT ' . $cols . ' FROM dtb_staff ' . $where;
        } else if ($csv_id == '9') {
            // 社員の場合
            $sql = 'SELECT ' . $cols . ' FROM dtb_work_possible_date ' . $where;
        }
        // 固有処理ここまで
