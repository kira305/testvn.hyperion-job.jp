...
class SC_FormParam {
            ...
            foreach ($this->arrCheck[$index] as $func) {
                ...
                switch ($func) {
                    ...
                    case 'FILE_NAME_CHECK_BY_NOUPLOAD':
                        $this->recursionCheck($this->disp_name[$index], $func, $value, $arrErr, $key, $this->length[$index]);
                        break;
                    // 小文字に変換
                    case 'CHANGE_LOWER':
                        $this->toLower($key);
     ...
     * @return void
     */
    function recursionCheck($disp_name, $func, $value, &$arrErr, $error_key, $length = 0, $depth = 0, $error_last_key = null
    ) {
        if (is_array($value)) {
            $depth++;
            foreach ($value as $key => $in) {
                $this->recursionCheck($disp_name, $func, $in, $arrErr, $error_key, $length, $depth, $key);
            }
        } else {
