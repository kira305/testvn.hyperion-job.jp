...
class GC_Utils {
	...
	if ($forLogInfo === true) {
	    $bklv = 3;
	    if (($traces[3]['class'] === 'LC_Page' || $traces[3]['class'] === 'LC_Page_Admin') && $traces[3]['function'] === 'log'
	    ) {
		$bklv = 4;
	    }
	}
	...
	foreach (array_reverse($arrBacktrace) as $arrLine) {
	    ...
	    if (strlen($arrLine['file']) === 0 && ($arrLine['class'] === 'SC_Helper_HandleError' || $arrLine['class'] === 'SC_Helper_HandleError_Ex') && ($arrLine['function'] === 'handle_error' || $arrLine['function'] === 'handle_warning')
	    ) {
		break 1;
	    }
	    ...
	    if (($arrLine['class'] === 'SC_Helper_HandleError' || $arrLine['class'] === 'SC_Helper_HandleError_Ex') && ($arrLine['function'] === 'handle_error' || $arrLine['function'] === 'handle_warning')
	    ) {
		break 1;
	    }
	    if (($arrLine['class'] === 'SC_Utils' || $arrLine['class'] === 'SC_Utils_Ex') && $arrLine['function'] === 'sfDispException'
	    ) {
		break 1;
	    }
	    if (($arrLine['class'] === 'GC_Utils' || $arrLine['class'] === 'GC_Utils_Ex') && ($arrLine['function'] === 'gfDebugLog' || $arrLine['function'] === 'gfPrintLog')
	    ) {
		break 1;
	    }
	}
    ...
    function gfMynumberLog($event, $detail) {
	if ($_SESSION['authority'] == 4 || $_SESSION['authority'] == 5) {
	    $today = date('Y/m/d H:i:s');
	    $msg = "[$today] [user=" . $_SESSION['login_name'] . " auth=" . $_SESSION['authority'] . " from={$_SERVER['REMOTE_ADDR']} login_id=" . $_SESSION['login_id'] . "] [$event] [$detail]\n";
	    error_log($msg, 3, DATA_REALDIR . "logs/mynumber.log");
	}
    }
