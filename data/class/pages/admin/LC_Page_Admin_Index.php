...
class LC_Page_Admin_Index extends LC_Page_Admin_Ex {
	...
	$objQuery = & SC_Query_Ex::getSingletonInstance();
	//メンバー情報取得
	$cols = 'member_id, authority, login_date, name, agency_id, corporate_id';
	$table = 'dtb_member';
	$where = 'login_id = ?';
	$arrData = $objQuery->getRow($cols, $table, $where, array($login_id));
	// セッション登録
	$sid = $this->lfSetLoginSession($arrData['member_id'], $login_id, $arrData['authority'], $arrData['name'], $arrData['login_date'], $arrData['agency_id'], $arrData['corporate_id']);
	// ログイン情報記録
     ...
     */
    function lfSetLoginSession($member_id, $login_id, $authority, $login_name, $last_login, $agency_id, $corporate_id) {
	...
	$objSess->SetSession('agency_id', $agency_id);
	$objSess->SetSession('corporate_id', $corporate_id);
	...
	GC_Utils_Ex::gfMynumberLog('login', 'lastlogin=' . $last_login);
