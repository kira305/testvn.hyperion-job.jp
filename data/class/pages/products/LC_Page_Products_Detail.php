...
class LC_Page_Products_Detail extends LC_Page_Ex {
    ...
    function init() {
        ...
        $this->arrPRODUCTMODE = $masterData->getMasterData('mtb_product_mode');
    }
...
//                    else
//                        SC_Response_Ex::sendRedirect(SHOPPING_URL . '?direct=on');
                    if ($objCustomer->isLoginSuccess(true))
            ...
            case 'cart':
                $this->arrErr = $this->lfCheckError($this->mode, $this->objFormParam, $this->tpl_classcat_find1, $this->tpl_classcat_find2);
                if (count($this->arrErr) == 0 && !in_array($product_id, $_SESSION['keep_product_list'])) {
        ...
        // kept
        $this->tpl_kept = false;
        if (isset($_SESSION['keep_product_list']) && in_array($product_id, $_SESSION['keep_product_list']))
            $this->tpl_kept = true;
        ...
        if (strlen($this->arrProduct['employment_status']) > 0)
            $this->arrProduct['employment_status'] = str_split($this->arrProduct['employment_status']);
        if (strlen($this->arrProduct['appeal_point']) > 0)
            $this->arrProduct['appeal_point'] = str_split($this->arrProduct['appeal_point']);
