...
class SC_CustomerList extends SC_SelectSql_Ex {
    ...
    function __construct($array, $mode = '') {
        ...
        if (is_array($this->arrSql['search_job'])) {
			foreach($this->arrSql['search_job'] as $data){
				$this->setWhere('job LIKE ?');
				$search_job = $this->addSearchStr($data);
            	$this->arrVal[] = mb_ereg_replace('[ 　]+','',$search_job);
			}
        }

		if (!isset($this->arrSql['search_bank'])) $this->arrSql['search_bank'] = '';
        if (strlen($this->arrSql['search_bank']) > 0) {
            $this->setWhere('bank LIKE ?');
            $search_bank = $this->addSearchStr($this->arrSql['search_bank']);
            $this->arrVal[] = mb_ereg_replace('[ 　]+','',$search_bank);
        }

        if (!isset($this->arrSql['search_account_holder'])) $this->arrSql['search_account_holder'] = '';
        if (strlen($this->arrSql['search_account_holder']) > 0) {
            $this->setWhere('(' . $dbFactory->concatColumn(array('account_holder1', 'account_holder2')) . ' LIKE ?)');
            $search_account_holder = $this->addSearchStr($this->arrSql['search_account_holder']);
            $this->arrVal[] = mb_ereg_replace('[ 　]+','',$search_account_holder);
        }

        if (!isset($this->arrSql['search_account_number'])) $this->arrSql['search_account_number'] = '';
        if (strlen($this->arrSql['search_account_number']) > 0) {
            $this->setWhere('account_number LIKE ?');
            $search_account_number = $this->addSearchStr($this->arrSql['search_account_number']);
            $this->arrVal[] = mb_ereg_replace('[ 　]+','',$search_account_number);
        }
